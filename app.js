...
document.addEventListener("DOMContentLoaded", () => {
    ...
    console.log("BiometricAuth =", window.Capacitor?.Plugins?.BiometricAuth);
...
/* ================= BIOMETRIC STATUS - BN HABEEB DATA HUB ================= */
...
function showDebug(message,error=false){
    ...
    const status=el("biometricStatus");
    ...
    if(status){
        ...
        <div style="display:flex;align-items:center;gap:8px;margin-bottom:6px;">
            ...
            <strong style="color:#14b8b6;">
                ${APP_NAME}
            </strong>
        </div>

        <div style="
            color:${error?"#ff4d4d":"#14b8b6"};
            font-size:12px;
...
async function checkBiometricStatus() {
    ...
    if (isNativeApp()) {
        ...
        try {
            ...
            showDebug(
                "Unable to access Android biometric.",
                true
            );
    ...
    if (!window.isSecureContext) {
        ...
        showDebug(
            "HTTPS is required for biometric authentication.",
            true
        );
    ...
    try {
        ...
        if (!token) {
            ...
            showDebug(
                "Login first before enabling biometric."
            );
        ...
        const response = await fetch(
            API + "/api/auth/webauthn/check-enabled",
            {
                method: "GET",
                headers: {
...
async function enableBiometric() {
    ...
    /* ==========================================================
       ANDROID APP (Capgo Native Biometric)
    ========================================================== */
    ...
    if (isNativeApp()) {
        ...
        try {
            ...
            // Save only the flag.
            // JWT is already stored after normal login.
    ...
    if (!biometricReady) {
        ...
        showDebug("Preparing passkey...");

        try{

            const response=await fetch(
                API+"/api/auth/webauthn/register-start",
                {
                    method:"POST",
                    headers:{
                        Authorization:
    ...
    try{
        ...
        const options=cachedRegOptions;
        ...
        const finish=
        await fetch(
            API+"/api/auth/webauthn/register-finish",
            {
...
async function loginWithBiometric() {
    ...
    if (isNativeApp()) {
        ...
        try {
            ...
            showDebug("Biometric authentication successful.");
...
async function loginWithBiometric() {
  ...
  if (isNative) {
    btn.disabled = true;
    btn.innerHTML = `<img src="${APP_LOGO}" style="width:20px;height:20px;margin-right:8px;">Touch sensor...`;
    ...
    try {
      ...
      if (getToken()) {
        ...
        showMsg("Please login normally first.", "error");
      }

    } catch (err) {
      showDebug(err.message || "Authentication cancelled", true);
    }

    btn.disabled = false;
    btn.innerHTML = `<img src="${APP_LOGO}" style="width:20px;height:20px;margin-right:8px;">Login with Fingerprint`;
    return;
  }
  ...
  try {
    ...
    showDebug(err.message || "Login failed", true);
  }
...
async function openPurchaseModal(planId, planName, planPrice) {
  ...
  if (bioBtn) {
    const isWebAuthn = window.PublicKeyCredential && window.isSecureContext;
    if (!isWebAuthn) {
      ...
      try {
        ...
        bioBtn.style.display = data.enabled ? 'flex' : 'none';
      } catch (e) {
...
function openAirtimePin() {
  ...
  if (!isWebAuthn) {
    ...
    }).catch(() => { if (bioBtn) bioBtn.style.display = 'none'; });
  }
...
async function purchaseWithBiometric() {
  ...
  // SAFETY CHECK: Only run if WebAuthn exists
...
/* ================= BUY DATA - WITH BNHABEEB RECEIPT ================= */
async function buyData(pin) {
  ...
  try {
    ...
    if (res.ok && data.success !== false) {
      ...
      // Show BNHABEEB receipt
      showReceipt({
        number: data.phone || phone,
        network: data.network || selectedNetwork?.toUpperCase(),
        plan: data.plan_name || selectedPlan?.name,
        amount: data.amount, // use API response to avoid NaN
        date: data.created_at || new Date().toISOString(),
        txnId: data.reference || data.transaction_id || data.id || 'BNH' + Date.now().toString().slice(-8),
...
/* ================= BUY AIRTIME - WITH BNHABEEB RECEIPT ================= */
  ...
  try {
    ...
    if (res.ok && data.success !== false) {
      ...
      showReceipt({
        ...
        plan: 'Airtime Top-up',
        amount: data.amount || amount, // prefer API amount
        date: data.created_at || new Date().toISOString(),
        txnId: data.reference || data.transaction_id || data.id || 'BNH' + Date.now().toString().slice(-8),
        status: data.status || 'SUCCESS'
      });
